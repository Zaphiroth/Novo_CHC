...
ProjectSample <- function(imp.total, 
                          hospital.universe) {
  ...
  out.data <- proj.data %>% 
    filter(!(pchc %in% hospital.universe$pchc)) %>% 
    mutate(flag_sample = 1)
  
  
  ##---- Sample province projection ----
  ## district mapping
  district.mapping <- hospital.universe %>% 
  ...
  proj.sample <- district.mapping %>% 
    ...
    ## sample flag
    left_join(hospital.universe, by = c('province', 'city', 'district')) %>% 
    ## sample pack
